...
raceYear = 4
dat.76 = data.frame(CBdata.1_10[raceYear])
subset(dat.76, select = -c(PiS.TiS))
dat.76
```
...
dat.76.clean = dat.76
tabLen = nrow(dat.76.clean)

for(x in 1:tabLen){
  if(is.na(dat.76[(tabLen+1) - x, 3]) == TRUE){
    dat.76.clean = dat.76.clean[-((tabLen+1) -x), ]
  }
}
...
dat.76.clean2 = dat.76
dat.76.clean2 = subset(dat.76, !is.na(Age))
dat.76.clean2
...
filter.func = function(data){
  cleanData = data
  cleanData = subset(data, !is.na(Age))
}

dat.76.clean3 = filter.func(dat.76)
identical(dat.76.clean3, dat.76.clean2)
```
...
```{r combine_dat}
allData = data.frame()
for(x in 1:length(CBdata.1_10)){
  yearData = data.frame(CBdata.1_10[x])
  subset(yearData, select = -c(PiS.TiS))
  yearData = filter.func(yearData)
  allData = rbind(allData, yearData)
  
  if(x == 4){
    print(identical(yearData, dat.76.clean))
  }
  
  if(5 < x){
    print(summary(yearData))
  }
}
...
a) How does the average of the recorded ages in 1976 compare to that same average over the entire `CBdata.1_10` data set?
 - the average age overall went up
...
b) Recall that the `CBdata.1_10` contains the first ten year's worth of cherry blossom race data. How does the average participant age over the first five years compare to the average age over years 6-10?
 - the average age went from an average of 31.89 to 34.88
